...
3. [Table Schemas](#3-table-schemas)
   ...
   - 3.12 [Multilayer feature table](#312-multilayer-feature-table)
4. [Datasets and GEO Accessions](#4-datasets-and-geo-accessions)
5. [BTM Immune Groups](#5-btm-immune-groups)
6. [Vaccine Types](#6-vaccine-types)
7. [Color Palettes](#7-color-palettes-defined-in-requiredr)
...
| **DMD** | Duchenne Muscular Dystrophy (biological negative control) |
...
Generated by `Modelling/6_Statistical_Modelling.Rmd` (mouse-to-human
...
**Best model per task.** The **random forest** is the best algorithm for human
rank transfer (R² = 0.234) and directional concordance (ROC-AUC = 0.849); the
**lasso** is marginally best for shared-LEG classification (ROC-AUC 0.617
predictive and 0.627 explanatory; PR-AUC 0.915 and 0.917). The models shipped for
each task are `rf_model_rank.rds`, `rf_model_direction.rds` and
...
### 3.12 Multilayer feature table

Generated by `Modelling/6_Statistical_Modelling.Rmd`. One row per gene × condition, assembled from the DGE, sequence-evolution and regulatory-annotation layers.

| Column | Type | Description |
|--------|------|-------------|
| `hgnc_symbol` | chr | Human gene symbol (HGNC) |
| `treatment`, `pathogen`, `timepoint_comparison` | chr | Gene × condition key |
| `log2fc_mouse`, `se_mouse`, `inverse_se_mouse` | num | Mouse log₂FC, standard error and 1/SE |
| `rank_mouse` | num | Mouse absolute rank (0–100), percentile of `|Log2FC| x -log10(adj P)` |
| `rank_mouse_btm` | num | Mouse absolute rank computed within BTMs |
| `protein_identity` | num | Ensembl protein identity (%) between human and mouse |
| `dist_k80` | num | Kimura 2-parameter (K80) CDS distance |
| `tf_total_human` | int | Total human TF ChIP-seq binding events for the gene |
| `tf_shared_pct` | num | Percentage of those TFs that also bind the mouse ortholog |
| `cre_pls`, `cre_pels`, `cre_dels` | int | Counts of PLS, pELS and dELS cCREs at the human locus |
| `cre_*_match_pct` | num | Percentage of cCREs matching between species, by type |
| `ctcf_bound`, `ctcf_match` | num | CTCF binding status and CTCF-matched cCRE counts |
| `btm_group` | chr | BTM module membership used as a feature |

Targets modelled per gene are `shared` (Shared vs Mouse-only LEG), `rank_human` (absolute rank, 0–100) and `sign_concordant` (directional concordance).

---

## 4. Datasets and GEO Accessions

Curated blood transcriptome studies used in the manuscript. Mouse and human cohorts were matched by tissue, antigen, vaccine or challenge type, and time points.

| Condition | Organism | Accession | Platform | Platform ID |
|-----------|----------|-----------|----------|-------------|
| Influenza (Fluad) + Hepatitis B (Engerix B) | Mouse | GSE120661 | Agilent SurePrint G3 Mouse GE 8x60K | GPL10787 |
...
| *S. aureus* infection | Mouse | GSE19668 | Affymetrix Mouse Genome 430 2.0 | GPL1261 |
| *S. aureus* infection | Human | GSE33341 | Affymetrix Human Gene 1.0 ST | GPL6244 |
| *E. coli* infection | Mouse | GSE33341 | Affymetrix Mouse Genome 430 2.0 | GPL1261 |
| *E. coli* infection | Human | GSE33341 | Affymetrix Human Gene 1.0 ST | GPL6244 |
| Burn | Mouse | GSE7404 | Affymetrix Mouse 430.2 | GPL1261 |
| Burn | Human | GSE37069 | Affymetrix Human Genome U133 Plus 2.0 | GPL570 |
| Trauma | Mouse | GSE7404 | Affymetrix Mouse Genome 430 2.0 | GPL1261 |
| Trauma | Human | GSE36809 | Affymetrix Human Gene 1.0 ST | GPL6244 |
| Duchenne muscular dystrophy (negative control) | Mouse | GSE1025 | Affymetrix Murine Genome U74A v2 | GPL81 |
| Influenza (dirty mice, not used) | Mouse | GSE182858 | Illumina MouseWG-6 v2.0 | GPL6887 |
...
## 7. Color Palettes (defined in `required.R`)
