...
PAL <- c("#2e9db7", "#4361ee", "#7b8fe0", "#6b9e5f", "#d98a35", "#c94f86",
         "#3f9c86", "#8b6fd6", "#b0563f", "#4f9dd9", "#9b7a3f", "#7a4f9b")
...
```{r}
#| fig-height: 6
btm <- read_data("fig4-btm-modules.json") |>
  filter(is.finite(nes_mouse), is.finite(nes_human))

ggplotly(
  ggplot(btm, aes(nes_mouse, nes_human, color = group, shape = sig,
                  text = paste0(process, "<br>", pathogen, " · ", timepoint_comparison, "<br>", sig))) +
    geom_point(size = 2.2, alpha = .9) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "black", linewidth = .4) +
    facet_wrap(~ pathogen, ncol = 3, scales = "free") +
    scale_color_manual(values = PAL) +
    scale_shape_manual(values = c("all modules" = 16, "significant in mice" = 17, "significant in both" = 18)) +
    labs(x = "NES mouse", y = "NES human", color = NULL, shape = NULL) +
    theme_minimal(base_size = 11),
  height = 560, tooltip = "text")
```
...
```{r}
#| fig-height: 7
legs <- read_data("fig4-btm.json") |>
  filter(is.finite(percent), !is.na(process)) |>
  group_by(pathogen, process) |>
  summarise(pct = mean(percent, na.rm = TRUE), group = first(group), .groups = "drop")

ggplotly(
  ggplot(legs, aes("", pct, color = group,
                   text = paste0(process, "<br>", round(pct), "% shared"))) +
    geom_boxplot(width = .25, outlier.shape = NA, color = "grey35", fill = "grey92") +
    geom_jitter(width = .16, size = 1.1, alpha = .9) +
    facet_wrap(~ pathogen, ncol = 3, scales = "free_y") +
    scale_color_manual(values = PAL) +
    labs(x = "", y = "% shared", color = NULL) +
    theme_minimal(base_size = 11) +
    theme(axis.text.x = element_blank()),
  height = 520, tooltip = "text")
```
...
```{r}
#| fig-height: 4
cre <- read_data("fig6-cre-type.json") |>
  mutate(ctcf = ifelse(ctcf_bound, "CTCF-bound", "not-CTCF"),
         type = factor(type, levels = c("PLS", "pELS", "dELS")))

ggplotly(
  ggplot(cre, aes(n, abs_diff, color = ctcf,
                  text = paste0(gene, "<br>|dLog2FC| ", round(abs_diff, 2), "<br>", n, " CREs"))) +
    geom_point(size = .8, alpha = .5) +
    facet_wrap(~ type, ncol = 3, scales = "free_x") +
    scale_color_manual(values = c(`CTCF-bound` = PAL[2], `not-CTCF` = PAL[3])) +
    labs(x = "CREs per gene", y = "|Δlog2FC|", color = NULL) +
    theme_minimal(base_size = 11),
  height = 340, tooltip = "text")
```
